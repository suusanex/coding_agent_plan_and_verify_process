...
Runner 0.2.3のレビュー判断を評価するための手動シナリオである。通常のunit / integration testはprovider呼び出しと結果保存の契約を検証し、実モデルの検出力は証明しない。この文書のシナリオは未実施であり、過去のsession継続実験を今回のPASSとして再利用しない。
