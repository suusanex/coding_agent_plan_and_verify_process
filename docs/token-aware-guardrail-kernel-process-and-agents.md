...
Readiness and architecture artifacts record repository ref/commit, upstream artifact revisions or hashes, artifact revision, and evaluation time. A semantic change to any baseline source or inspected production evidence makes the verdict `stale`, even when paths are unchanged.
