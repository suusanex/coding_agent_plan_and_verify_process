...
> Status update (2026-07-14): この文書の `slice-impl` / `standard-implementer` 記述は、delegation must化を導入した時点の historical baseline です。Issue #44 以降の標準実装 route は、各非自明な READY scope / slice を `high-implementation-starter` から開始し、valid `READY_FOR_STANDARD_COMPLETION` handoff 後だけ `standard-implementation-completer` を使い、`NEEDS_HIGH_MODEL_REENTRY` は HIGH_MODEL に戻します。旧 agent 名は explicit legacy compatibility invocation に限ります。現行 contract は package skills、state / audit templates、`docs/codex-first-routing-branching.md` を参照してください。
