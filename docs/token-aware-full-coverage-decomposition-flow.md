...
- Architecture residuals assigned to a slice retain stable item ID, classification, decision owner, human-input requirement, blocking state, resolution phase, and source-backed next action in the Living Record `Unresolved Decision Ownership` table. A non-blocking `SliceLocalContract` is resolved by the implementation-contract owner, not promoted to a human prerequisite merely because its concrete address is greenfield.
