...
description: Classify the requested change, identify high-risk runtime boundaries, and recommend the minimum sufficient process profile without implementing anything.
...
- 利用可能であれば、このタスクに対応する既存の Plan または docs
...
| `full-coverage` | 変更が broad、ambiguous、または強く相互接続されており、複数の runtime sequence が関係し、recovery semantics が重要、または過去の試行で sequence / production-binding gap が露出している場合 |
...
- Selected contracts / IDs: <選択した Contract IDs>
- Files inspected: <一覧>
...
profile を推奨し、selected contracts を列挙したら停止してください。
