...
- **Implementation route propagation**: `implementation_route: adaptive` / `implementation_route_source: default` を既定とする。Design Pair は upstream durable artifact に `implementation_route: design-pair` / `implementation_route_source: explicit-user-selection` と explicit user evidence がある場合だけ保持する。risk や architecture から自動選択、推奨、提案してはいけない。
