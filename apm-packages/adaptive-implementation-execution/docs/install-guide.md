...
補助スクリプトの実行はskillの使用を強制しません。skillは利用者が Adaptive Implementation Execution を明示指定した場合だけ選択され、通常の「実装して」「このPlanを実装して」だけでは選択されません。frontmatter の `disable-model-invocation: true` により model 判断での暗黙起動を禁止し、`user-invocable: true` により利用者の明示起動は維持します。選択後の実行順序、HIGH_MODELとSTANDARD_MODELの役割、handoff、re-entry、verification boundaryは `SKILL.md` がsource of truthです。
