...
`design-pair-implementation-execution` を利用者が明示選択した場合、この package は tracked Design Pair handoff を追加 input として受け取ります。`READY_FOR_ADAPTIVE_IMPLEMENTATION` だけでなく、`Interaction stage: complete`、Target Map の提示・選択要求、提示後の actual user response、non-empty selection または explicit all-Adaptive delegation、pending Target なしを編集前に検証します。`AWAITING_USER_INPUT`、空の selected set、上流文書から再構成した user evidence は受理せず、`BLOCKED / BlockedByInvalidCompletionHandoff` で停止します。
