...
Assert-Contains $skill 'parent / routerであることはimplementation ownershipを意味しません' 'parent topology boundary'
...
Assert-NotContains $decisionOwner 'high-implementation-starter|standard-implementation-completer|READY_FOR_STANDARD_COMPLETION|COMPLETED_BY_HIGH_MODEL|Direct completion reason|HIGH_MODEL code changes' 'removed decision-closer vocabulary'
...
Assert-NotContains $handoff 'Decision closure|HIGH_MODEL code changes|non-local-decisions-closed|Legacy Adaptive handoff normalization' 'removed handoff vocabulary'
