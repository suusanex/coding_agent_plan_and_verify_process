...
4. 実装前に handoff review で実装許可と coverage ledger を確認する。
5. READY になった範囲だけ、内部的に `standard-implementer` へ委譲して実装する。
6. 実装後に `standard-verifier` などで test / verification / close 可否を確認する。
7. 止まる必要がある場合だけ、必要最小限の質問を返す。
