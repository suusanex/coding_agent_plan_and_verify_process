...
- parent / slice-prep / Adaptive Implementation separation is needed
...
- Every non-trivial READY slice implementation starts with `high-implementation-starter` / `HIGH_MODEL`.
- Decision-free bounded completion may move to `standard-implementation-completer` / `STANDARD_MODEL` after a valid handoff; re-entry returns to HIGH_MODEL.
...
- `DELEGATED_IMPLEMENTATION` requires every non-trivial READY slice to have observed `high-implementation-starter` evidence.
...
- Update it at major checkpoints and delegation boundaries, including start, ExecutionMode decision, slice-prep and Adaptive Implementation batch boundaries, parent review gate, cross-slice verification, residual decision, planned handoff, tool switch, model switch, and emergency checkpoint.
