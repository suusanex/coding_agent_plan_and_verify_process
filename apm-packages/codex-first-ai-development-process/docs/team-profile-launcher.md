...
For VS Code Codex extension in repository-local operation, use `scripts/apply-codex-first-local.cs` first so that `AGENTS.md`, `.codex`, `.agents/skills`, and `templates` are created in the target repository.
