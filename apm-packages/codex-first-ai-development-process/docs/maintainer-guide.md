...
- implementation route はdurable route、resume、Design Pair evidenceがないfresh intakeだけ`adaptive / default`で初期化される。resumeでは両route fieldが必須であり、Design Pairではhandoff path、interaction stage、Target Map presentation / post-map user evidenceも必須である。欠落、矛盾、waiting中のREADY正規化はAdaptiveへ補完せず停止する。唯一の互換例外はcanonical `Legacy Adaptive handoff normalization`を満たすexact pre-Design-Pair Adaptive completion handoffである。Design Pairはexplicit user selectionの場合だけhandoff review後かつHIGH start前に実行され、初回Target Map提示後と未確定disposition時にturnを停止する。production code / tests editは禁止され、post-map confirmation evidenceのあるLocked DecisionsだけがDesign Pair由来のbinding decisionである。upstream binding constraintsは別sectionで伝播する。
