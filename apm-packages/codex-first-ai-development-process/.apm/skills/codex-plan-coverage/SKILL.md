...
Use this skill only by forwarding into `codex-first-cost-router`, preserving the same source of truth, state artifact, and audit artifact.

## Workflow

1. Read the user's request, issue body, PR comment, branch state, or supplied artifact as the source of truth.
2. Locate or create `plans/<slug>/codex-first-state.md`; create or update `plans/<slug>/codex-first-audit.md` when delegation evidence or close audit is needed.
...
6. Preserve Routing Plan, Edit Permission, audit artifact path, Agent Usage Ledger, and DelegationCompliance.
7. Record resume-critical gate result back to the state artifact, and record delegation / model / close-audit detail in the audit artifact.
