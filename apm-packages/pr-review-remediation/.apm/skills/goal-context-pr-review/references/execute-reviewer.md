...
On failure, final `*.raw.md` is not published. Failure statuses include `timeout`, `auth_failure`, `non_zero_exit`, `empty_output`, `malformed_output`, `process_start_failure`, `input_delivery_failure`, `unsupported`, and `failed`. Failures are never interpreted as “no findings”.
...
- timeout / auth / non-zero / empty / malformed / process-start / stdin-delivery / write-detected never mean “no findings”
...
| timeout / auth / empty / stdin-delivery | Treat as execution failure; do not assess as no findings |
