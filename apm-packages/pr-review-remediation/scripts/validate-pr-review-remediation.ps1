...
Assert-Contains $sharedPlanTemplate 'Thread mode: single-round / role-thread-reuse / portable-handoff' 'single-round-compatible handoff mode choices'
Assert-Contains $sharedPlanTemplate '基礎版のsingle-round利用ではrole thread bindingを要求せず' 'single-round role binding non-requirement'
...
Assert-Contains $goalSkill 'role-thread-reuse' 'default role task reuse mode'
Assert-Contains $goalSkill 'rebind-thread' 'approved role task rebinding'
Assert-Contains $goalSkill 'portable-handoff' 'explicit portable handoff fallback'
...
Assert-Contains $cycleManager 'RoleThreadReuseMode = "role-thread-reuse"' 'role thread reuse vocabulary'
Assert-Contains $cycleManager 'PortableHandoffMode = "portable-handoff"' 'portable handoff vocabulary'
...
Assert-Contains $cycleManager 'rebind-thread' 'role task rebind operation'
Assert-Contains $cycleManager 'ThreadUri' 'manager-derived Codex task URI'
Assert-Contains $cycleManager 'binding history' 'durable role binding history validation'
...
Assert-Contains 'tests/pr-review-remediation/PRR-003/scenarios.json' 'late-implementation-thread-binding' 'PRR-003 late Implementation Thread binding path'
Assert-Contains 'tests/pr-review-remediation/PRR-003/scenarios.json' 'portable-cold-start' 'PRR-003 explicit portable fallback path'
...
Assert-Contains 'tests/pr-review-remediation/manual-model-smoke/README.md' '(?s)role-thread-reuse.*Review Thread ID.*Implementation Thread ID.*portable-handoff' 'role task binding and portability smoke contract'
