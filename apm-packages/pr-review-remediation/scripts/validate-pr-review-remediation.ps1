...
Assert-Contains $executor 'PublishPair' 'paired atomic raw and metadata publication'
Assert-Contains $executor 'stdin:prompt|stdin' 'Codex prompt delivered off argv'
Assert-Contains $executor 'available-tools' 'Copilot read-only tool allowlist'
Assert-Contains $executor 'disable-builtin-mcps' 'Copilot built-in MCP disabled'
Assert-Contains $executor 'write_detected|WorktreeSnapshot' 'worktree write detection'
Assert-Contains $executor 'FindApmCanonicalAgents|apm_modules' 'APM installed agent contract resolution'
Assert-Contains $executor 'empty_output|malformed_output|timeout|auth_failure|process_start_failure' 'distinct execution failure statuses'
Assert-Contains $executor 'partial-' 'partial artifact naming'
Assert-Contains $executor 'Production code changed: No' 'required production marker validation'
