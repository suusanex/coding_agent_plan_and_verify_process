...
if (parsed.Help || parsed.Error is not null)
{
    ...
    Console.WriteLine("dotnet run --file scripts/codex-notification-runtime/install-codex-notification-runtime-local.cs -- [install] [--dry-run] [--check] [--codex-home <path>] [--install-root <path>]");
    return;
}
if (parsed.SelfTest)
{
...
var runtimeSourceRoot = FindRuntimeSourceRoot();
...
static string? FindRuntimeSourceRoot([CallerFilePath] string installerSourcePath = "")
{
