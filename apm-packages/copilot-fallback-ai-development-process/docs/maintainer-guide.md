...
`high-implementation-starter`と`standard-implementation-completer`のcanonical contractとCopilot frontmatterはrepository root agents / Adaptive packageがsource of truthです。fallback local installerはこの2 filesをrepository rootから直接コピーし、package内に同名templateを持ちません。Adaptive validatorはsingle-source copy経路とmirror不在を検証します。

同名fileのcollision policyは、同内容ならno-op、異なる未管理fileは既定で保持、明示`--force`だけが置換です。APM導入fileはAPM lockfile、fallback local installerで配置したfileのcontent sourceは実行時のcanonical repository rootです。local installerは利用者変更を誤削除しないためremoveを自動化せず、Adaptive APM ownershipへのmigrationでは人が差分を確認します。
...
- fallback固有custom agentsに必要な`name`、`description`、`tools`、`model`、`handoffs`があり、canonical Adaptive agentsはCodex向けlossy変換を避けるため`tools`を省略している
...
- Adaptive validatorがroot canonical agentsのsingle-source copy経路とfallback mirror不在を検証する
...
- installer dry-run と適用後確認で canonical root agents と Adaptive state fields が配布される
