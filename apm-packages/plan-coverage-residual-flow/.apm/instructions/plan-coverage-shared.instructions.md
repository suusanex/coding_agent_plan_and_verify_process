...
- **Adaptive implementation ownership**: after implementation authorization, every non-trivial bounded implementation starts with `decision-surface-implementation-owner`. It owns the implementation feedback loop — code inspection, production code, tests, wiring, focused verification, and consequence re-assessment — for as long as an unresolved decision surface remains, and it may transfer only a `READY_FOR_BOUNDED_RESIDUAL_IMPLEMENTATION` handoff once every decision surface concern is `Resolved` or source-backed `N/A`. `bounded-residual-implementation-owner` owns only the bounded residual completion that fits inside locked contracts and semantics, and returns `NEEDS_DECISION_SURFACE_REENTRY` only when a new or previously locked decision surface must be reopened. These write owners run serially within one bounded pass. The split axis is the remaining decision surface, not a fixed division by role, model cost, or parent/subagent topology.
- **Explicit implementation route selection**: initialize `implementation_route: adaptive` and `implementation_route_source: default` only at fresh intake when no durable route or resume evidence exists. Use `implementation_route: design-pair` only when the user explicitly selected it, and preserve `implementation_route_source: explicit-user-selection` through durable artifacts and resume state. On resume, missing or contradictory route metadata must fail closed and must not be normalized to Adaptive. There is no legacy-handoff compatibility exception; an old-schema, old-agent-name, or mixed old/new completion handoff fails closed as an invalid completion handoff. Do not infer, recommend, or propose Design Pair from risk, difficulty, size, or architecture. When explicitly selected, run `design-pair-implementation-execution` only after implementation authorization and before `decision-surface-implementation-owner`.
