...
7. Preserve `implementation_route`, `implementation_route_source`, `design_pair_handoff`, and `design_pair_interaction_stage` in the parent decomposition artifact and each implementation-ready slice handoff. A Design Pair route waiting at target selection, disposition confirmation, upstream decision, or artifact repair must not advance to Adaptive or verification.
8. Every non-trivial implementation-ready slice starts with `high-implementation-starter.agent.md`; a valid `READY_FOR_STANDARD_COMPLETION` handoff may invoke `standard-implementation-completer.agent.md`. Run `verification-kernel.agent.md` independently for each implemented slice.
9. After the required slices have been verified, run `cross-slice-verification-kernel.agent.md`. Route unresolved coverage or explicit FixNow candidates through `coverage-gap-triage.agent.md` and `coverage-gap-resolution-slice.agent.md` as applicable, then run `residual-decision-gate.agent.md` for the parent close decision.
