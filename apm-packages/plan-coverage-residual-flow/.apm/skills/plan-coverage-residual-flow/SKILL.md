...
The `design-pair-implementation-execution` package is a separate package with formal targets `copilot`, `codex`, and `agent-skills`. Plan Coverage can use the Design Pair route when both packages are installed for the same target and the user explicitly selects Design Pair. Keep `plan-coverage-residual-flow` selection evidence and Design Pair implementation route selection evidence separate. While Design Pair is waiting, do not fall back to Adaptive.
