...
This skill does not replace the individual agents. Agent-specific rules, output formats, and verdict vocabulary are authored once in the Plan Coverage package-owned canonical agent definitions. APM projects those contracts into runtime-specific agent surfaces. At runtime, invoke the deployed agent by its logical name. Do not treat `.github/agents` or `.codex/agents` as independent contract authorities. Do not require a source checkout of `apm-packages/.../.apm/agents/` as a runtime dependency.
...
9. Implement only after the handoff review or a documented equivalent Inline Ready Gate allows implementation for the bounded parent Plan pass. The Plan Coverage parent/router that sequences this flow is a separate orchestrator and topology concern from the Adaptive semantic owners it invokes; do not conflate parent/subagent invocation shape or model-tier assignment with `decision-surface-implementation-owner` / `bounded-residual-implementation-owner` semantic ownership.
   ...
   After this optional pre-stage, every non-trivial pass starts with `decision-surface-implementation-owner.agent.md`; do not classify shape need from documents or route directly to the bounded-residual owner. The split axis is whether an unresolved decision surface remains, not a fixed role or model-tier assignment; parent/subagent invocation topology is a separate concern from this semantic ownership.
   - `READY_FOR_BOUNDED_RESIDUAL_IMPLEMENTATION`: validate the complete Bounded Residual Implementation Handoff, then run `bounded-residual-implementation-owner.agent.md` serially.
   - `CONTINUE_DECISION_SURFACE_IMPLEMENTATION`: continue the same decision-surface-implementation-owner run when possible; use it as a resume state only at an execution boundary.
   - `IMPLEMENTATION_COMPLETED`: aggregate the implementation evidence and continue to verification.
   - `NEEDS_DECISION_SURFACE_REENTRY`: stop the bounded-residual owner, preserve its re-entry handoff and current worktree, then return serially to `decision-surface-implementation-owner.agent.md`.
   ...
   Use `none` for a confirmed non-applicable binding and `unknown` only when the source artifact does not resolve it. The active implementation agent returns the current-phase rows; it does not rewrite prior phase rows. The orchestrator is the single aggregation owner: it merges phase deltas by stable Change ID into the `Implementation Self-Map` in `plans/<slug>-implementation-execution.md`, alongside phase owners, verdict sequence, checks, acceptance evidence, and Remaining Work. Keep the Bounded Residual Implementation Handoff inline unless resume, another thread/model, or another worker requires a tracked `plans/<slug>-bounded-residual-implementation-handoff.md`.
...
11. Every non-trivial implementation-ready slice starts with `decision-surface-implementation-owner.agent.md`; a valid `READY_FOR_BOUNDED_RESIDUAL_IMPLEMENTATION` handoff may invoke `bounded-residual-implementation-owner.agent.md`. Adaptive agents keep their standalone output contract and return implementation evidence plus `Implementation Self-Map Delta`. The Plan Coverage parent aggregates those results into `Implementation Evidence` and `Implementation Self-Map`; Adaptive is not a Living Record writer and no normal `*-implementation-execution.md` file is created. Before invoking Adaptive, the parent selects either inline-only handoff or a registered tracked-handoff path. Keep the Bounded Residual Implementation Handoff inline by default. If a cross-thread/model transition may be used, the parent must first apply a matching `Artifact Exceptions` row to that slice's Living Record with reason code `cross-thread-handoff`, the exact `plans/<slug>-slice-SL-xxx-bounded-residual-implementation-handoff.md` path, supplemental status, owner, and lifecycle, then pass that registered path to Adaptive. Without the pre-applied row, Adaptive may not create the tracked file or transfer to the bounded-residual owner; it must continue on the decision-surface owner or stop with an exception request for the parent.

    When invoking the bounded-residual owner in Slice Living Record mode, the parent passes `artifact_mode: slice-living-record`, `living_record_path`, the proposed exact `reentry_handoff_path: plans/<slug>-slice-SL-xxx-decision-surface-reentry-handoff.md`, and `output_contract: parent-persisted-handoff-payload`. `NEEDS_DECISION_SURFACE_REENTRY` uses delayed registration because the re-entry need is unknown before the bounded-residual owner runs. It returns the complete Decision-Surface Re-entry Handoff as an unpersisted parent payload and must not create its tracked file. The Plan Coverage parent validates the verdict and route identity, applies an `Artifact Exceptions` row with reason code `cross-thread-handoff` and that exact path, persists the payload at the registered path, and only then resumes `decision-surface-implementation-owner.agent.md`. The row lifecycle must state this payload -> exception row -> parent persistence -> decision-surface-owner resume order. If the row cannot be applied, no re-entry file is saved and the decision-surface owner is not resumed.
...
14. After all required slices are independently verified, create `plans/<slug>-full-coverage-close.md` from `references/full-coverage-close.md` and run `cross-slice-verification-kernel.agent.md`.
    - For `CROSS_SLICE_VERIFIED` or `CROSS_SLICE_VERIFIED_WITH_RESIDUAL_DECISION_REQUIRED`, apply the cross-slice section and ledger delta, then run `residual-decision-gate.agent.md`, apply its section and ledger delta, and only then determine close readiness.
    - For `CROSS_SLICE_PARTIAL_WITH_FIX_CANDIDATES`, apply the partial cross-slice delta but do not run `residual-decision-gate.agent.md`. Record the trigger in the close record's `FixNow Repair Loop`, route each selector to an affected Slice Living Record, run `coverage-gap-triage.agent.md` in section-delta mode unless a complete direct selector permits bypass, run one bounded `coverage-gap-resolution-slice.agent.md` pass, apply its `Gap Repair Evidence` and Coverage Ledger Delta, rerun `verification-kernel.agent.md` for every affected slice, then rerun `cross-slice-verification-kernel.agent.md`. Residual Decision may start only after the rerun returns a verdict that permits it and all repair / re-verification deltas are applied.
    - A selector that cannot be mapped to one bounded target slice, a repair that remains broader than `fix-slice`, or a repeated unresolved candidate stops the automatic repair loop and is routed by the existing blocking / residual vocabulary instead of being silently retried.
    These agents retain separate semantics and verdict vocabularies even though their canonical state is aggregated into Living Records and the close record.
...
- current decision-surface / bounded-residual implementation owner and verdict sequence, once implementation has started
- Bounded Residual Implementation Handoff status and decision-surface re-entry reason, when applicable
