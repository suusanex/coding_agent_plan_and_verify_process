...
The `design-pair-implementation-execution` package is a separate package with formal targets `copilot`, `codex`, and `agent-skills`. When both packages are installed for the same target and the user explicitly selects Design Pair, Plan Coverage may invoke that route and must keep `plan-coverage-residual-flow` selection evidence separate from Design Pair implementation route selection evidence. While Design Pair is waiting, do not fall back to Adaptive. Plan Coverage parent runtime qualification of the Design Pair -> Adaptive path on GitHub Copilot CLI is not claimed by Design Pair package support alone; complete that qualification in the Plan Coverage Copilot CLI issue.
...
5. Treat every executable `plans/<slug>-slice-SL-xxx.md` artifact as a bounded Plan that re-enters this Plan Coverage flow. Read the parent Plan, approved readiness / architecture source, decomposition artifact, and the slice artifact without re-deciding shared architecture or work assigned to another slice.
6. Run the standard pre-implementation gates required by that slice's recommended profile. Preserve parent FR / AC, Case IDs, XC IDs, field continuity, and explicit residuals in the slice artifacts and normal Plan Coverage outputs.
7. Before an implementation handoff may authorize any executable slice, the Plan Coverage parent owns an `Architecture baseline compatibility` check:
   ...
   - Record Slice ID, Readiness verdict, Baseline authority, Baseline identity, Observed semantics, `Match / Drift / Unclear`, and Required action in the implementation handoff review or an explicitly equivalent gate.
...
8. Preserve `implementation_route`, `implementation_route_source`, `design_pair_handoff`, and `design_pair_interaction_stage` in the parent decomposition artifact and each implementation-ready slice handoff. A Design Pair route waiting at target selection, disposition confirmation, upstream decision, or artifact repair must not advance to Adaptive or verification.
9. Every non-trivial implementation-ready slice starts with `high-implementation-starter.agent.md`; a valid `READY_FOR_STANDARD_COMPLETION` handoff may invoke `standard-implementation-completer.agent.md`. Run `verification-kernel.agent.md` independently for each implemented slice.
10. After the required slices have been verified, run `cross-slice-verification-kernel.agent.md`. Route unresolved coverage or explicit FixNow candidates through `coverage-gap-triage.agent.md` and `coverage-gap-resolution-slice.agent.md` as applicable, then run `residual-decision-gate.agent.md` for the parent close decision.
