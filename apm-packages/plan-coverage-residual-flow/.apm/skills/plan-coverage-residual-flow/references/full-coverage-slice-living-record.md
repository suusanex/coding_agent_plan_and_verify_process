...
The exception row records the exact slice-scoped path, reason code, why the Living Record is insufficient, owner, canonical/supplemental status, and lifecycle, and the parent applies the row before file creation. The separate artifact is supplemental unless the existing contract explicitly requires otherwise. It must not replace the parent Plan, Slice Living Record, or canonical Coverage Ledger. Different slices may progress in parallel because they have different records; multiple writers must not edit the same slice record concurrently. Before Adaptive is invoked, a tracked `plans/<slug>-slice-SL-xxx-bounded-residual-implementation-handoff.md` requires a pre-applied `cross-thread-handoff` row and the registered path must be supplied to Adaptive; an inline handoff does not. A tracked `plans/<slug>-slice-SL-xxx-decision-surface-reentry-handoff.md` uses delayed registration because its trigger is not known in advance: the bounded-residual-implementation-owner returns an unpersisted payload, the parent applies the exact-path row, the parent persists the payload, and only then may the decision-surface-implementation-owner resume. The payload is not a repository artifact before that gate completes.
