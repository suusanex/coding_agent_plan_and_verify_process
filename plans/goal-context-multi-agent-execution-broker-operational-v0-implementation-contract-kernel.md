...
| Local IPC / worker ownership | Windows named pipe and Job Object | .NET BCL / Windows API | Confirmed | same-user ACL、request/response JSON frames。Host は `KILL_ON_JOB_CLOSE` Job Object の唯一の owner とし、worker tree をjobへassignする。 |
...
- Remaining work: `NotImplementedOrMismatch`: production code/tests/docs/wiring。`ManualOnly`: actual Codex App/Copilot real-issue Early Operational Trial。
