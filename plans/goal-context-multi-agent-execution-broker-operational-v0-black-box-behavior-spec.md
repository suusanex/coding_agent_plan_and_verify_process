...
| SRC-BRK-014 | 最初に正式対応する provider は実能力を証拠付きで選ぶ | capability gate | 同「前提・再検証事項」「意図的に後続設計へ残している事項」 | Copilot CLI は候補だが作業仮説を確定扱いしない |
...
| AX-BRK-005 | output | stdout / stderr / empty / large or incremental | 保存・取得結果が変わる | output 欠落を成功と偽装しない |
...
| CASE-BRK-004 | SRC-BRK-004,005,011 | worker が exit 0 で終了する | terminal observed fact、exit code、stdout/stderr、agent-reported result の有無を区別して保存する | exit 0 を Issue の意味的完全成功と断定しない | Defined |
| CASE-BRK-005 | SRC-BRK-004,005 | worker が nonzero または launch failure になる | failure 種別、利用可能な diagnostics、部分 output を保存し取得できる | run を正常終了と偽装しない | Defined |
...
| CASE-BRK-011 | SRC-BRK-008,009,014 | provider capability gate を満たす adapter を選ぶ | v0 対応 provider と接続方式が evidence と共に登録され、同じ Broker surface から実行できる | 未確認の作業仮説だけで正式対応を宣言しない | Defined |
...
| CASE-BRK-014 | SRC-BRK-012 | 本物の開発 Issue を eligible provider に委譲する | 開始、非同期継続、終了認識、output 回収、次行動の判断まで一連で実行できる | fixture-only / fake-only で v0 完了としない | Defined |
...
| INV-BRK-006 | fake-only / process-start-only evidence は Operational v0 完了証拠にならない | 014 | real development Issue E2E が必要 |
...
| URI-BRK-002 | SRC-BRK-003,011 | durable store と provider-neutral terminal event の exact schema | No（Plan behavior は確定） | architecture / runtime contract で authority、compatibility、atomicity を確定 |
| URI-BRK-003 | SRC-BRK-001,002 | Codex App facade と長寿命 Broker の exact local transport / startup | No（Plan behavior は確定） | architecture readiness / implementation contract で production address と lifecycle を確定 |
...
- Case IDs: `CASE-BRK-001`〜`CASE-BRK-015`
...
- Decisions made: Operational v0 の completion unit を start→durable observation→terminal notification→result retrieval→next action とした。provider選定、store、transportは behavior ではなく downstream evidence-backed contract decision とした。
- Excluded combinations: `EX-BRK-001`〜`EX-BRK-005`
