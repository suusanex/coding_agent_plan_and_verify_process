...
11. Every non-trivial implementation-ready slice starts with `high-implementation-starter.agent.md`; a valid `READY_FOR_STANDARD_COMPLETION` handoff may invoke `standard-implementation-completer.agent.md`. Adaptive agents keep their standalone output contract and return implementation evidence plus `Implementation Self-Map Delta`. The Plan Coverage parent aggregates those results into `Implementation Evidence` and `Implementation Self-Map`; Adaptive is not a Living Record writer and no normal `*-implementation-execution.md` file is created. Before invoking Adaptive, the parent selects either inline-only handoff or a registered tracked-handoff path. Keep the Implementation Completion Handoff inline by default. If a cross-thread/model transition may be used, the parent must first apply a matching `Artifact Exceptions` row to that slice's Living Record with reason code `cross-thread-handoff`, the exact `plans/<slug>-slice-SL-xxx-implementation-completion-handoff.md` path, supplemental status, owner, and lifecycle, then pass that registered path to Adaptive. Without the pre-applied row, Adaptive may not create the tracked file or delegate to STANDARD; it must continue on HIGH or stop with an exception request for the parent.

    When invoking STANDARD in Slice Living Record mode, the parent passes `artifact_mode: slice-living-record`, `living_record_path`, the proposed exact `reentry_handoff_path: plans/<slug>-slice-SL-xxx-high-model-reentry-handoff.md`, and `output_contract: parent-persisted-handoff-payload`. `NEEDS_HIGH_MODEL_REENTRY` uses delayed registration because the re-entry need is unknown before STANDARD runs. STANDARD returns the complete High-model Re-entry Handoff as an unpersisted parent payload and must not create its tracked file. The Plan Coverage parent validates the verdict and route identity, applies an `Artifact Exceptions` row with reason code `cross-thread-handoff` and that exact path, persists the payload at the registered path, and only then resumes `high-implementation-starter.agent.md`. The row lifecycle must state this payload -> exception row -> parent persistence -> HIGH resume order. If the row cannot be applied, no re-entry file is saved and HIGH is not resumed.
